...
-(void)updateRateView
{
    ...
    self.frame = CGRectMake(self.frame.origin.x, self.frame.origin.y,
                            self.count*_starSize, _starSize);
...
-(void)setStarSize:(CGFloat)starSize
{
    ...
    for(int i=1; i<=self.count; i++)
    {
        Star* star = (Star*)[self viewWithTag:TagOffset+i];
        star.size = _starSize;
        star.frame = CGRectMake((i-1)*starSize + _duraingSize * (i-1), 0, _starSize, _starSize);
    }
}

- (void)setDuraingSize:(CGFloat)duraingSize
{
    _duraingSize = duraingSize;
    ...
    for(int i=1; i<=self.count; i++)
    {
        Star* star = (Star*)[self viewWithTag:TagOffset+i];
        star.size = _starSize;
        star.frame = CGRectMake((i-1)*_starSize + _duraingSize * (i-1), 0, _starSize, _starSize);
    }
}
...
-(void)setFrame:(CGRect)frame
{
    ...
    if(frame.size.width != self.count*_starSize || frame.size.height != _starSize)
    {
        frame.size.width = self.count*_starSize;
...
-(void)handleTouches:(NSSet*)touches
{
    if(_canRate)
    {
        ...
        if(x < 0.0f)
            ...
            float div = (self.frame.size.width * self.step) / self.count;
            x = (x / div) + self.step;
            x = div * (int)x;
        }
...
- (CGSize)intrinsicContentSize {
    return CGSizeMake(self.starSize * self.count, self.starSize);
}
