...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (!self.onlyDisplayText && [imagePath isKindOfClass:[NSString class]]) {
        ...
        }else if (!([imagePath hasPrefix:@"http"] && [imagePath hasPrefix:@"https"])) {//易元江修改
            NSString *imagehttpPathUrl = [NSString stringWithFormat:@"http://%@",imagePath];
            [cell.imageView sd_setImageWithURL:[NSURL URLWithString:imagehttpPathUrl] placeholderImage:[UIImage imageNamed:@"gd"] completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, NSURL *imageURL) {
                //在这里输出下error
                //                NSLog(@"-----------%@",error);
                NSString *imagehttpsPath= [NSString stringWithFormat:@"https://%@",imagePath];
                [cell.imageView sd_setImageWithURL:[NSURL URLWithString:imagehttpsPath] placeholderImage:self.placeholderImage];
            }];
        }else {
